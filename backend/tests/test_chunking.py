...
def test_empty_messages_produce_no_chunks() -> None:
    assert chunk_messages([], vod_id=99, channel_id=1, twitch_vod_id="987654") == []


def test_fewer_messages_than_window_produce_one_chunk() -> None:
    chunks = chunk_messages(_messages(10), vod_id=99, channel_id=1, twitch_vod_id="987654")
    assert len(chunks) == 1
...
def test_exactly_one_window_produces_one_chunk() -> None:
    chunks = chunk_messages(_messages(50), vod_id=99, channel_id=1, twitch_vod_id="987654")
    assert len(chunks) == 1
...
def test_windows_overlap_by_configured_amount() -> None:
    ...
    chunks = chunk_messages(_messages(90), vod_id=99, channel_id=1, twitch_vod_id="987654")
    assert len(chunks) == 2
    assert chunks[0].start_message_id == 1
...
def test_custom_window_and_overlap() -> None:
    chunks = chunk_messages(
        _messages(15), vod_id=99, channel_id=1, twitch_vod_id="987654", window=10, overlap=5
    )
    assert len(chunks) == 2
    assert chunks[0].start_message_id == 1
    assert chunks[0].end_message_id == 10
...
def test_chunk_text_joins_username_and_message() -> None:
    chunks = chunk_messages(_messages(2), vod_id=99, channel_id=1, twitch_vod_id="987654")
    assert chunks[0].text == "user0: message 0\nuser1: message 1"


def test_chunk_carries_vod_and_channel_ids() -> None:
    chunks = chunk_messages(_messages(3), vod_id=42, channel_id=7, twitch_vod_id="987654")
    assert chunks[0].vod_id == 42
    assert chunks[0].channel_id == 7
    assert chunks[0].twitch_vod_id == "987654"


def test_chunk_offsets_and_timestamps_span_the_window() -> None:
    chunks = chunk_messages(_messages(5), vod_id=99, channel_id=1, twitch_vod_id="987654")
    chunk = chunks[0]
